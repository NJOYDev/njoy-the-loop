...
// import autoshipHandlers from './custom/autoship';
...
export default class Cart extends PageManager {
    ...
    cartUpdate($target) {
        ...
        switch($target.prop('tagName')) {
            case 'BUTTON': newQty = $target.data('action') === 'inc' ? oldQty + 1 : oldQty - 1; break;
            case 'INPUT': newQty = parseInt($target.val())
        }

        if (newQty === null) {
            alert("ERROR: CAN'T UPDATE CART QUANTITY")
            return
        }
    ...
    bindCartEvents() {
        ...
        const cartRemoveItem = _.bind(_.debounce(this.cartRemoveItem, debounceTimeout), this);
            ...
            // cart update - text input change
            $('.form-input--incrementTotal', this.$cartContext).on('change', e => {
                cartUpdate($(e.target))
            })
